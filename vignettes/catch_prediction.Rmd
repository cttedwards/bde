...
knitr::opts_chunk$set(fig.path = 'fig/bde-', fig.width = 6, tidy = TRUE, tidy.opts = list(blank = TRUE, width.cutoff = 105), message = FALSE, warning = FALSE, collapse = TRUE, comment = "#>")
...
```{r}
# save results
res <- posterior(mdl.fit, pars = "predicted_catch", dim.names = list(iter = 1:nit, year = X.dimnames$year))
```
...
sim <- extract(mdl.fit, pars = "DBN")[[1]][,2]

discrepancy[["BN"]] <- data.frame(iter = 1:nit, observed = emp, simulated = sim)

emp <- extract(mdl.fit, pars = "DLN")[[1]][,1]
sim <- extract(mdl.fit, pars = "DLN")[[1]][,2]

discrepancy[["LN"]] <- data.frame(iter = 1:nit, observed = emp, simulated = sim)
```
...
lims <- range(discrepancy[["BN"]][,2:3])
plot(simulated ~ observed, discrepancy[["BN"]], xlim = lims, ylim = lims, xlab = "Observed vs. Expected", ylab = "Simulated vs. Expected")
abline(0, 1, lwd = 2, col = 2)
...
lims <- range(discrepancy[["LN"]][,2:3])
plot(simulated ~ observed, discrepancy[["LN"]], xlim = lims, ylim = lims, xlab = "Observed vs. Expected", ylab = "Simulated vs. Expected")
abline(0, 1, lwd = 2, col = 2)
...
```{r, echo = FALSE}
res <- res / 1e6
gg <- ggplot(melt(res), aes(year, value)) + 
        ...
        labs(x = "Year", y = "Incidental catch (thousand tonnes)") +

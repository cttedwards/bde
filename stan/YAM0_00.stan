...
transformed data {
	...
	for (i in 1:N[1]) {
		...
		eff_estimate_sum[XY_sample[i], XA_sample[i], XM_sample[i]] += eff_sample[i];
...
generated quantities {
	...
	// results
	int  bin_sim_obs[Y, A, M];
	real pos_sim_obs[Y, A, M];
	...
	for(i in 1:N[1]) {
		...
		mu_log      = beta0 + betaY[XY_sample[i]] + betaA[XA_sample[i]] + betaM[XM_sample[i]];
		
		bin_hat = inv_logit(theta_logit);
		pos_hat = bin_hat * exp(mu_log + square(sigma[XY_sample[i]]) / 2);
				
		bin_hat_sum[XY_sample[i], XA_sample[i], XM_sample[i]] += bin_hat;
		pos_hat_sum[XY_sample[i], XA_sample[i], XM_sample[i]] += pos_hat;
	}
	...
	for(i in 1:N[1]) {
		...
		bin_sim_sum[XY_sample[i], XA_sample[i], XM_sample[i]] += bin_sim;
		pos_sim_sum[XY_sample[i], XA_sample[i], XM_sample[i]] += pos_sim;
	}

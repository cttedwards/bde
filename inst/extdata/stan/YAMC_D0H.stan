...
model {
    ...
    for(i in 1:N_nz) {
    	...
    	mu_log[i] = beta0 + betaY[XY_sample_nz[i]] + betaA[XA_estimate_nz[i]] + betaM[XM_sample_nz[i]] + betaC[XC_sample_nz[i]];
...
generated quantities {
	...
	for(i in 1:N[1]) {
		...
		mu_log      = beta0 + betaY[XY_sample[i]] + betaA[XA_estimate[i]] + betaM[XM_sample[i]] + betaC[XC_sample[i]];
		...
		for (j in 1:A[1]) {
		    for (k in 1:M) {
		        for (l in 1:C) {
        			...
        			mu_log      = beta0  + betaY[i] + betaA[area_to_area[j]] + betaM[k] + betaC[l];
